...
class ManyHotEncoder:
    ...
    """
    def __init__(self, labels, sample_rate, hop_length, pooling_time_ratio, n_frames=None):
        ...
        # print(self.labels, self.sample_rate, self.hop_length, self.pooling_time_ratio, self.n_frames)
        # print("end\n")
        # exit()
    ...
    def encode_weak(self, labels):
        ...
        if type(labels) is pd.Series:
            if labels.empty:
                labels = []
            elif {"event_labels"}.issubset(labels.index):
                labels = labels["event_labels"].split(",")
        ####################################################
    ...
    def encode_strong_df(self, label_df):
        ...
        if type(label_df) is pd.DataFrame:
            ...
            # else:
            #     if {"event_labels"}.issubset(label_df.columns):
            #         for label in label_df["event_labels"]:
            #             print(label)
            #         exit()
            ...
            if type(label_df) is pd.Series:
                ...
                #############################################################
                    ...
                    if not pd.isna(label_df["event_labels"]):
                        for label in label_df["event_labels"].split(","):
                            # print(label_df)
                            i = self.labels.index(label)
                            onset_float = float(label_df["filename"].replace(".wav", "").split("_")[-2])
                            offset_float = float(label_df["filename"].replace(".wav", "").split("_")[-1])
                            if onset_float == 0:
                                onset = int(onset_float * self.sample_rate // self.hop_length // self.pooling_time_ratio)
                                offset = int(offset_float * self.sample_rate // self.hop_length // self.pooling_time_ratio)
                                # print(onset, offset)
                                # exit()
                                y[onset:offset, i] = 1
    ...
    def state_dict(self):
        ...
        return {"labels": self.labels,
                ...
                "pooling_time_ratio": self.pooling_time_ratio,
                "n_frames": self.n_frames}
        ######################################################
    ...
    def load_state_dict(cls, state_dict):
        ...
        pooling_time_ratio = state_dict["pooling_time_ratio"]
        ######################################################
        n_frames = state_dict["n_frames"]
        # print(labels, sample_rate, hop_length, pooling_time_ratio, n_frames)
        # exit()
        return cls(labels, sample_rate, hop_length, pooling_time_ratio, n_frames)
...
class AverageMeterSet:
    ...
    def __str__(self):
        ...
        for name, meter in self.meters.items():
            ...
            # string += "{} {:{format}} \t".format(name, meter, format=format)
...
def get_transforms_AANPT(frames, scaler=None, add_axis_conv=True, augment_type=None):
    transf = []
    unsqueeze_axis = None
    if add_axis_conv:
        unsqueeze_axis = 0
...
def get_transforms_nopad(scaler=None, add_axis_conv=True, augment_type=None):
    transf = []
    unsqueeze_axis = None
    if add_axis_conv:
        unsqueeze_axis = 0
    ...
    transf.extend([ApplyLog(), ToTensor(unsqueeze_axis=unsqueeze_axis)])
    # if scaler is not None:
    #     transf.append(Normalize(scaler=scaler))
