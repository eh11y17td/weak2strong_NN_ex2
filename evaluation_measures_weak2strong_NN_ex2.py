...
import config_weak2strong_NN_ex2 as cfg
...
def get_f_measure_by_class(torch_model, nb_tags, dataloader_, thresholds_=None):
    ...
    for counter, (batch_x, y) in enumerate(dataloader_):
        ...
        pred_strong, pred_weak, pred_sof = torch_model(batch_x)
        ###################################################
        # pred_weak = pred_weak.cpu().data.numpy()
        prepare = Prepare(batch_x, None)
        prepare.initialize(pred_strong, pred_sof)
        pred_weak = prepare.new_weak.cpu().data.numpy()
        ###################################################
        labels = y.numpy()
...
def get_predictions(model, valid_dataset, decoder, pooling_time_ratio=1, save_predictions=None):
    for i, (input, _) in enumerate(valid_dataset):
        ...
        pred_strong, _ , _ = model(input.unsqueeze(0))
        ...
        ##################################
        # LOG.debug("filename{}: {}".format(i, valid_dataset.filenames.iloc[i]))
        ##################################
        if i == 0:
