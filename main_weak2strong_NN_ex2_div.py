...
import config_weak2strong_NN_ex2_div as cfg
...
if __name__ == '__main__':
    ...
    #############################################################################
    filenames_train_w2s = weak2strong_df.filename.drop_duplicates().sample(frac=0.8, random_state=26)
    train_w2s_df = weak2strong_df[weak2strong_df.filename.isin(filenames_train_w2s)]
    valid_w2s_df = weak2strong_df.drop(train_w2s_df.index).reset_index(drop=True)
    
    valid_syn_w2s_df = pd.concat([valid_synth_df, valid_w2s_df]).reset_index(drop=True)
    #############################################################################
    ...
    #############################################################################
    train_w2s_df.onset = train_w2s_df.onset * cfg.sample_rate // cfg.hop_length // pooling_time_ratio
    train_w2s_df.offset = train_w2s_df.offset * cfg.sample_rate // cfg.hop_length // pooling_time_ratio
    #############################################################################

    # print(train_synth_df)
    # exit()
    LOG.debug(valid_synth_df.event_label.value_counts())
    LOG.debug(valid_w2s_df.event_label.value_counts())
    ...
    #     batch_sizes = [6*cfg.batch_size//15, cfg.batch_size//15, 8*cfg.batch_size//15]
    #     strong_mask = slice(6*cfg.batch_size//15 + cfg.batch_size//15, cfg.batch_size)
    ...
    valid_weak_data = DataLoadDf(valid_weak_df, dataset.get_feature_file, many_hot_encoder.encode_weak,
                                 transform=transforms_valid)
    #############################################################################
    valid_w2s_data = DataLoadDf(valid_w2s_df, dataset.get_feature_file, many_hot_encoder.encode_strong_df,
                                 transform=transforms_valid)
    #############################################################################
    ...
    for epoch in range(cfg.n_epoch):
        ...
        #############################################################################
        predictions_syn = get_predictions(crnn, valid_synth_data, many_hot_encoder.decode_strong, pooling_time_ratio,
                                      save_predictions=None)

        predictions_w2s = get_predictions(crnn, valid_w2s_data, many_hot_encoder.decode_strong, pooling_time_ratio,
                                      save_predictions=None)

        predictions = pd.concat([predictions_syn, predictions_w2s]).reset_index(drop=True)
        # print(predictions_syn)
        # print(predictions_w2s)
        # print(predictions)
        # print(valid_synth_df)
        # print(valid_w2s_df)
        # print(valid_syn_w2s_df)
        # exit()
        valid_events_metric = compute_strong_metrics(predictions, valid_syn_w2s_df)
        #############################################################################
